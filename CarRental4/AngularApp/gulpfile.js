...
gulp.task("ng-bump-version", function () {
  ...
  var env_file_path;
  if (ng_env) {
    env_file_path = path.join(env_dir_path, `environment.${ng_env}.ts`);
  }
  else {
    env_file_path = path.join(env_dir_path, `environment.ts`);
  }
  return gulp
    .src([env_file_path])
    .pipe(
      replace(/version:\s?"\d+.\d+.\d+",?/g, function (match) {
        ...
        console.log(
          `${this.file.path
          } New version to be written: ${replaced_version_string}`
        );
